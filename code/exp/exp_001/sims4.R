...
## ASSUMPTION:
## all preterm births are high-risk -- we can change proportions later
...
for (s in 1:n.sims){
  ...
  for (e in 1:length(err.lev)){ # e=1
    ...
    pm.err.hirisk <- rnorm(nrow(truedta_null), truedta_null$pm25, (err.lev[e]*10)*sd(truedta_null$pm25))
    
    pm.err <- ifelse(truedta_null$all_births > 0,
                     (truedta_null$term_births*pm.err.norisk + truedta_null$preterm_births*pm.err.hirisk)/(truedta_null$all_births),
                     mean(c(pm.err.norisk, pm.err.hirisk)))
    
    mod <- glm(preterm_births ~ pm.err + offset(log(all_births)),
               data = truedta_null, family=quasipoisson)
    
    diff.null.moreErr[s, c(index, (index+1))] <- summary(mod)$coefficients[2,1:2]
...
names(diff.null.moreErr) <- paste0(rep(c("b_", "se_"), each = length(err.lev)), err.lev)
