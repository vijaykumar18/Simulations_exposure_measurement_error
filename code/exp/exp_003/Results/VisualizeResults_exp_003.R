setwd("/scratch/user/u.vk230134/Downloads/DiffMeasError/CodeReview/exp/exp_003/Results/")
...
plot(c(1, dim(harm.res)[1]), c(0.95,1.30), pch = '', las = 1,
...
bias_null$perc.bias<-round(bias_null$perc.bias,digits = 2)

summary(bias_null)
summary(bias_assoc)
# Define color limits based on the type of bias
assoc_limits <- c(-100, 40) # Adjust these limits for association
null_limits <- c(-0.25, 0.25)      # Adjust these limits for null association
...
# Create the heatmaps separately
...
p2 <- ggplot(bias_null, aes(x = bias_type, y = error_level, fill = round(perc.bias,2))) +
  geom_tile(color = "white") +
  custom_scale_fill("Null Association") +
  geom_text(aes(label = sprintf("%.1f%%", round(perc.bias, 2))), size = 4) +
...
p2
# Combine the plots
pdf("ass_bias_perc_heatmap_045.pdf", width = 8, height = 6)
p1
dev.off()

pdf("null_bias_perc_heatmap_045.pdf", width = 8, height = 6)
p2
dev.off()
rm(list=ls())
