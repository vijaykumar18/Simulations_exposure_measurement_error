...
diff.harm.moreErr50     <- matrix(NA, nrow = n.sims, ncol = length(err.lev) * 2)  # Store b and se
...
for (s in 1:n.sims){ 
  ...
  for (e in 1:length(err.lev)){ # e=1
    ...
    diff.harm.moreErr50[s, c(index, (index+1))] <- summary(mod)$coefficients[2,1:2]
...
names(diff.harm.moreErr50) <- paste0(rep(c("b_", "se_"), each = length(err.lev)), err.lev)
